...
if 'user' not in st.session_state:
    ...
    watchlist_ids = requests.get(f'http://127.0.0.1:5002/user/{st.session_state["user"]["id"]}/watchlist').json()

    movies = []
    for id in watchlist_ids:
        movies.append(requests.get(f"http://127.0.0.1:5001/movie/{id}").json())
    ...
    if add_watched:
        ...
        url = f'http://127.0.0.1:5002/user/{st.session_state["user"]["id"]}/watched_movie'
